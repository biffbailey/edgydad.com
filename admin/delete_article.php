...
if ((isset($_GET['id_article'])) && ($_GET['id_article'] != "") && (isset($_POST['button_delete']))) {
  $deleteSQL = sprintf("DELETE FROM blog_articles WHERE id_article=%s",
                       GetSQLValueString($_GET['id_article'], "int"));

  mysql_select_db($database_BlogConnection, $BlogConnection);
  $Result1 = mysql_query($deleteSQL, $BlogConnection) or die(mysql_error());

  $deleteGoTo = "list_articles.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
<div class="col3">
    ...
    <div class="content">
        <h2 class="tagLine">Confirm Delete Article!</h2>
    </div>
    <p>The following article will be deleted from the database if you confirm. If you don't want to delete the article, use the back button or navigate to another page without clicking on the Delete button.</p>
    <form id="form1" name="form1" method="post" action="">
      <table width="90%" border="0" cellpadding="1">
        <tr>
          <td><?php echo $row_rsArticle_to_Delete['description_topic']; ?>&gt;<?php echo $row_rsArticle_to_Delete['title_article']; ?></td>
          <td><?php echo $row_rsArticle_to_Delete['id_topic']; ?></td>
        </tr>
        <tr>
<td><?php echo $row_rsArticle_to_Delete['description_article']; ?></td>
          <td><?php echo $row_rsArticle_to_Delete['date_article']; ?></td>
        </tr>
        <tr>
          <td ><div align="center">
            <input name="button_delete" type="submit" id="button_delete" onclick="MM_callJS('article_deleted')" value="Confirm Delete" />
            </div>
          </td>
          <td ><div align="center">
            <input name="Cancel" type="button" onclick="MM_goToURL('parent','list_articles.php');return document.MM_returnValue" value="Cancel" />
          </div></td>
        </tr>
      </table>
    </form>
