...
if ((isset($_GET['id_topic'])) && ($_GET['id_topic'] != "") && (isset($_POST['button_delete']))) {
  $deleteSQL = sprintf("DELETE FROM blog_topics WHERE id_topic=%s",
                       GetSQLValueString($_GET['id_topic'], "int"));

  mysql_select_db($database_BlogConnection, $BlogConnection);
  $Result1 = mysql_query($deleteSQL, $BlogConnection) or die(mysql_error());

  $deleteGoTo = "list_topics.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
<div class="content">
  <h2 class="tagLine">Confirm Delete Topic:</h2>
</div>

<form method="post" name="form1" id="form1">
  <table align="center">
    <tr valign="baseline">
      <td width="112" align="right" nowrap="nowrap">Title_topic:</td>
      <td colspan="2"><input name="title_topic" type="text" value="<?php echo $row_rsTopic_to_Update['title_topic']; ?>" size="69" /></td>
    </tr>
    <tr valign="baseline">
      <td nowrap="nowrap" align="right">Description_topic:</td>
      <td colspan="2"><textarea name="description_topic" cols="32"><?php echo $row_rsTopic_to_Update['description_topic']; ?></textarea></td>
    </tr>
    <tr valign="baseline">
      <td nowrap="nowrap" align="right">&nbsp;</td>
      <td width="206"><input name="button_delete" type="submit" id="button_delete" onclick="MM_callJS('delete_confirm')" value="Confirm Delete" /></td>
      <td width="215"><input name="Button_Cancel" type="button" id="Button_Cancel" onclick="MM_goToURL('parent','list_topics.php');return document.MM_returnValue" value="Cancel, Return to List"  hspace="50" /></td>
    </tr>
  </table>
