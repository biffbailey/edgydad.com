...
if ((isset($_GET['id_coffee'])) && ($_GET['id_coffee'] != "") && (isset($_POST['button_delete']))) {
  $deleteSQL = sprintf("DELETE FROM coffee_green WHERE id_coffee=%s",
                       GetSQLValueString($_GET['id_coffee'], "int"));

  mysql_select_db($database_BlogConnection, $BlogConnection);
  $Result1 = mysql_query($deleteSQL, $BlogConnection) or die(mysql_error());

  $deleteGoTo = "coffee_log.php";
  if (isset($_SERVER['QUERY_STRING'])) {
...
$maxRows_rsCoffee_to_Delete = 10;
$pageNum_rsCoffee_to_Delete = 0;
if (isset($_GET['pageNum_rsCoffee_to_Delete'])) {
  $pageNum_rsCoffee_to_Delete = $_GET['pageNum_rsCoffee_to_Delete'];
}
$startRow_rsCoffee_to_Delete = $pageNum_rsCoffee_to_Delete * $maxRows_rsCoffee_to_Delete;

$colname_rsCoffee_to_Delete = "1";
if (isset($_GET['id_coffee'])) {
  $colname_rsCoffee_to_Delete = $_GET['id_coffee'];
}
mysql_select_db($database_BlogConnection, $BlogConnection);
$query_rsCoffee_to_Delete = sprintf("SELECT * FROM coffee_green WHERE id_coffee = %s", GetSQLValueString($colname_rsCoffee_to_Delete, "int"));
$query_limit_rsCoffee_to_Delete = sprintf("%s LIMIT %d, %d", $query_rsCoffee_to_Delete, $startRow_rsCoffee_to_Delete, $maxRows_rsCoffee_to_Delete);
$rsCoffee_to_Delete = mysql_query($query_limit_rsCoffee_to_Delete, $BlogConnection) or die(mysql_error());
$row_rsCoffee_to_Delete = mysql_fetch_assoc($rsCoffee_to_Delete);

if (isset($_GET['totalRows_rsCoffee_to_Delete'])) {
  $totalRows_rsCoffee_to_Delete = $_GET['totalRows_rsCoffee_to_Delete'];
} else {
  $all_rsCoffee_to_Delete = mysql_query($query_rsCoffee_to_Delete);
  $totalRows_rsCoffee_to_Delete = mysql_num_rows($all_rsCoffee_to_Delete);
}
$totalPages_rsCoffee_to_Delete = ceil($totalRows_rsCoffee_to_Delete/$maxRows_rsCoffee_to_Delete)-1;
?>
...
<div class="col3">
    ...
    <div class="content">
        <h2 class="tagLine">Confirm Delete Coffee!</h2>
      </div>
    <p>The following green coffee purchase will be deleted from the database if you confirm. If you don't want to delete the item, use the back button or navigate to another page without clicking on the Confirm Delete button.</p>
    <form id="form1" name="form1" method="post" action="">
      <table width="90%" border="0" cellpadding="1">
        <tr>
          <td><?php echo $row_rsCoffee_to_Delete['name_coffee']; ?></td>
<td><?php echo $row_rsCoffee_to_Delete['id_coffee']; ?></td>
        </tr>
        <tr>
<td><?php echo $row_rsCoffee_to_Delete['vendor_coffee']; ?></td>
<td><?php echo $row_rsCoffee_to_Delete['date_coffee']; ?></td>
        </tr>
        <tr>
          <td ><div align="center">
            <input name="Delete" type="submit" id="Delete" onclick="MM_callJS('article_deleted')" value="Confirm Delete" />
            </div>
          </td>
          <td ><div align="center">
            <input name="Cancel" type="button" onclick="MM_goToURL('parent','coffee_log.php');return document.MM_returnValue" value="Cancel" />
          </div></td>
        </tr>
      </table>
    </form>
...
mysql_free_result($rsCoffee_to_Delete);
?>
