...
if ((isset($_GET['id_comment'])) && ($_GET['id_comment'] != "") && (isset($_POST['delete_button']))) {
  $deleteSQL = sprintf("DELETE FROM blog_comments WHERE id_comment=%s",
                       GetSQLValueString($_GET['id_comment'], "int"));

  mysql_select_db($database_BlogConnection, $BlogConnection);
  $Result1 = mysql_query($deleteSQL, $BlogConnection) or die(mysql_error());

  $deleteGoTo = "manage_blog_comments.php";
...
if ((isset($_GET['id_comment'])) && ($_GET['id_comment'] != "") && (isset($_POST['button_delete']))) {
  $deleteSQL = sprintf("DELETE FROM blog_comments WHERE id_comment=%s",
                       GetSQLValueString($_GET['id_comment'], "int"));
...
$maxRows_rs_comment_to_delete = 10;
$pageNum_rs_comment_to_delete = 0;
if (isset($_GET['pageNum_rs_comment_to_delete'])) {
  $pageNum_rs_comment_to_delete = $_GET['pageNum_rs_comment_to_delete'];
}
$startRow_rs_comment_to_delete = $pageNum_rs_comment_to_delete * $maxRows_rs_comment_to_delete;

$colname_rs_comment_to_delete = "-1";
if (isset($_GET['id_comment'])) {
  $colname_rs_comment_to_delete = $_GET['id_comment'];
}
mysql_select_db($database_BlogConnection, $BlogConnection);
$query_rs_comment_to_delete = sprintf("SELECT * FROM blog_comments WHERE id_comment = %s", GetSQLValueString($colname_rs_comment_to_delete, "int"));
$query_limit_rs_comment_to_delete = sprintf("%s LIMIT %d, %d", $query_rs_comment_to_delete, $startRow_rs_comment_to_delete, $maxRows_rs_comment_to_delete);
$rs_comment_to_delete = mysql_query($query_limit_rs_comment_to_delete, $BlogConnection) or die(mysql_error());
$row_rs_comment_to_delete = mysql_fetch_assoc($rs_comment_to_delete);

if (isset($_GET['totalRows_rs_comment_to_delete'])) {
  $totalRows_rs_comment_to_delete = $_GET['totalRows_rs_comment_to_delete'];
} else {
  $all_rs_comment_to_delete = mysql_query($query_rs_comment_to_delete);
  $totalRows_rs_comment_to_delete = mysql_num_rows($all_rs_comment_to_delete);
}
$totalPages_rs_comment_to_delete = ceil($totalRows_rs_comment_to_delete/$maxRows_rs_comment_to_delete)-1;
?>
...
<div class="col3">
    ...
    <div class="content">
        <h2 class="tagLine">Confirm Delete Blog Comment!</h2>
    </div>
  ...
  <form id="form1" name="form1" method="post" action="">   
   <table width="90%" border="0" cellpadding="1">
      <tr>
        <td><?php echo $row_rs_comment_to_delete['name_comment']; ?></td>
        <td width="10%"><?php echo $row_rs_comment_to_delete['id_comment']; ?></td>
        <td width="10%"><?php echo $row_rs_comment_to_delete['id_article']; ?></td>
      </tr>
      <tr>
        <td><?php echo $row_rs_comment_to_delete['title_comment']; ?></td>
        <td colspan="2"><?php echo $row_rs_comment_to_delete['email_comment']; ?></td>
      </tr>
      <tr>
        <td><?php echo $row_rs_comment_to_delete['title_comment']; ?></td>
        <td colspan="2"><?php echo $row_rs_comment_to_delete['datetime_comment']; ?></td>
      </tr>
      <tr>
        <td colspan="3"><?php echo $row_rs_comment_to_delete['text_comment']; ?></td>
      </tr>
      <tr>
        <td><div align="center">
          <input name="delete_button" type="submit" id="delete_button" onclick="MM_callJS('article_deleted')" value="Confirm Delete" />
          <input type="submit" name="button_delete" id="button_delete" value="Submit" />
        </div></td>
        <td colspan="2"><div align="center">
          <input name="Cancel" type="button" onclick="MM_goToURL('parent','manage_blog_comments.php');return document.MM_returnValue" value="Cancel" />
        </div></td>
      </tr>
    </table>
...
mysql_free_result($rs_comment_to_delete);
?>
